    ...
    try {
      const conversations = await this.prisma.$queryRaw<any[]>`
        SELECT DISTINCT
          ...
          CASE
            WHEN m.sender_id = ${userId}::text THEN m.receiver_id
            ELSE m.sender_id
          END as participant_id,
          CASE
            WHEN m.sender_id = ${userId}::text THEN CONCAT(rp.first_name, ' ', rp.last_name)
            ELSE CONCAT(sp.first_name, ' ', sp.last_name)
          END as participant_name,
          CASE
            WHEN m.sender_id = ${userId}::text THEN rp.profile_picture_url
        ...
        INNER JOIN (
          SELECT
            job_id,
            CASE
              WHEN sender_id = ${userId}::text THEN receiver_id
              ELSE sender_id
          ...
          FROM messages
          WHERE sender_id = ${userId}::text OR receiver_id = ${userId}::text
        ) latest ON m.job_id = latest.job_id
          AND (
            (m.sender_id = ${userId}::text AND m.receiver_id = latest.other_user_id) OR
            (m.receiver_id = ${userId}::text AND m.sender_id = latest.other_user_id)
          )
          AND latest.rn = 1
        LEFT JOIN (
          SELECT
            ...
            COUNT(*) as count
          FROM messages
          WHERE receiver_id = ${userId}::text AND is_read = false
          GROUP BY job_id, sender_id
        ) unread_count ON m.job_id = unread_count.job_id
          AND ((m.sender_id = ${userId}::text AND m.receiver_id = unread_count.other_user_id) OR
               (m.receiver_id = ${userId}::text AND m.sender_id = unread_count.other_user_id))
        LEFT JOIN (
          SELECT
            job_id,
            CASE
          ...
          AND ((m.sender_id = ${userId}::text AND m.receiver_id = total_count.other_user_id) OR
               (m.receiver_id = ${userId}::text AND m.sender_id = total_count.other_user_id))
        WHERE (m.sender_id = ${userId}::text OR m.receiver_id = ${userId}::text)
