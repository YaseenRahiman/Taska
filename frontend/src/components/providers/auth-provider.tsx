  ...
  const register = async (data: any) => {
    try {
      ...
      if (result.accessToken) {
        ...
        // Wait for cookies to be fully written to browser storage with verification
        // Reduced iterations for faster response while still ensuring cookie persistence
        let cookieVerified = false;
        for (let i = 0; i < 5; i++) {
          await new Promise(resolve => setTimeout(resolve, 100));
          const cookies = document.cookie;
        ...
        if (!cookieVerified) {
          console.warn('[AuthProvider] Cookie verification timed out after 500ms, redirecting anyway');
        }
        ...
        // Use Next.js router for navigation to ensure Playwright can track redirects properly
        // Small delay ensures state updates are processed before navigation
        await new Promise(resolve => setTimeout(resolve, 100));
