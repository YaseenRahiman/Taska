...
test.describe('Credit System', () => {
  test.describe('Credit Balance', () => {
    ...
    test('should display current credit balance', async ({ page }) => {
      ...
      // Check for credit balance display - use separate selectors to avoid CSS/text selector mixing
      const balanceSection = page.locator('[data-testid="credit-balance"], .credit-balance').or(page.locator('text=/balance/i')).first();
    ...
    test('should display lifetime credits statistics', async ({ page }) => {
      ...
      // The page should render - check that body has content
      const bodyContent = await page.locator('body').innerHTML();
      const hasContent = bodyContent.length > 100; // Page rendered with some content
      expect(hasContent).toBe(true);

      // Check if we're on the credits page or redirected to login
      const url = page.url();
      const isValidPage = url.includes('/artisan/credits') || url.includes('/auth/login');
    ...
    test('should show credit transaction history', async ({ page }) => {
      ...
      // Look for transaction history section - use .or() to avoid CSS/text selector mixing
      const historySection = page.locator('[data-testid="transaction-history"], .transaction-history')
        .or(page.locator('text=/transactions|history/i')).first();
  ...
  test.describe('Auto Top-Up Configuration', () => {
    ...
    test('should display auto top-up settings', async ({ page }) => {
      ...
      const autoTopUpSection = page.locator('[data-testid="auto-topup"]')
        .or(page.locator('text=/auto.*top.*up|automatic.*purchase/i')).first();
...
test.describe('Boost System', () => {
  ...
  test.describe('Boost Activation', () => {
    ...
    test('should offer free boost option when available', async ({ page }) => {
      ...
      // Check for free boost option - use .or() to avoid CSS/text selector mixing
      const freeBoostOption = page.locator('text=/free boost|use free|free available/i')
        .or(page.locator('input[name*="free"], button:has-text("Free")')).first();
